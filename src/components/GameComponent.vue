...
<path d="M34.8255 50.4782L36.4163 56.3288" stroke="black" stroke-width="3.5" stroke-linecap="round"/>
        ...
        <div id="skip" @click="useSkip" :style="{color: currentGameComponent=='NextComponent'? '#fff' : (skips==0? '#909090' : '#000'), borderColor: currentGameComponent=='NextComponent'? '#fff' : (skips==0? '#909090' : '#000' )}">SKIP [{{skips}}]</div>
...
export default {
    ...
    data() {
        return {
            ...
            currentGameProperties: {nextDifficulty: 2 },
    ...
    computed: {
        ...
        currentGameEvents: function() {
            if (this.currentGameComponent === 'NextComponent') {
        ...
        transitionName() {
            return this.currentGameComponent === 'NextComponent' ? 'choice' : 'task';
        },
    ...
    methods: {
        ...
        useSkip(){
            if(this.skips > 0 && this.currentGameComponent === 'TaskComponent'){
                ...
                this.currentGameProperties = {nextDifficulty: this.currentDifficulty}
                this.currentGameComponent = 'NextComponent'
            }
        },
        abortMission() {
        ...
        returnHome(currentPoints, aborted) {
            ...
            if(currentPoints){
                ...
                this.currentGameProperties = {nextDifficulty: 3}
                this.currentGameComponent = 'NextComponent'
        ...
        updateState(points, nextDifficulty){
            this.currentPoints += points
            if (this.currentPoints >= this.missionPoints){
                this.returnHome(this.currentPoints, false)
            }
            else{
                this.currentGameProperties = {nextDifficulty: nextDifficulty}
                this.currentGameComponent = 'NextComponent'
            }
        }
    },

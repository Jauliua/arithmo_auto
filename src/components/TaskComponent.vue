...
let average_response_time = [6500, 16000, 21000, 35000, 53000, 62000, 72000]
let new_avg_response_time = 0
let nextDifficulty = 0
let points_left = 0
let nextDifficultyPoints = 0
...
export default {
    ...
    computed: {
        ...
        diff_average_response_times(){
            return this.$store.state.diff_average_response_times
        },
        diff_nr_task_answered(){
            return this.$store.state.diff_nr_task_answered
        }
    },
    methods: {
        handleKeydown(event) {
        ...
        findFirstGreaterThanPoints(target) {
            let left = 0;
        ...
        getNextDifficulty(){
            ...
            console.log('updated vuex log')
            if (this.tries_left <= 1){
                console.log('this.tries_left == 0')
                points_left = this.missionPoints - (this.currentPoints)
                nextDifficulty=  Math.max(this.difficulty-1,0)
            }
            else if (this.responseTime <= average_response_time[this.difficulty-1]*0.8){
                console.log('this.difficulty+1')
                points_left = this.missionPoints - (this.task_points+this.currentPoints)
                nextDifficulty=  Math.min(this.difficulty+1,7)
            }
            else if (this.responseTime >= average_response_time[this.difficulty-1]*1.2){
                console.log('this.difficulty-1')
                points_left = this.missionPoints - (this.task_points+this.currentPoints)
                nextDifficulty= Math.max(this.difficulty-1,0)
            }
        ...
        checkInput(){
            if(this.input_field == '' || this.tries_up_flag==true){
                ...
                if (this.input_field == this.data.result){
                    ...
                    this.input_field = ''
                    this.$emit('task-checked', this.task_points, this.getNextDifficulty())
                }
